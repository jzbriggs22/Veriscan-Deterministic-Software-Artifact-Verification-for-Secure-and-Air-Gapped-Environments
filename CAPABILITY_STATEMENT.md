...
| **In-Pipeline Mutation Detection** | Artifact SHA-256 is recorded immediately after acquisition and re-verified after the hash, signature, malware, and inspect stages — the stages that read artifact content. Any mutation of the artifact on disk between stages immediately aborts the run with a pipeline error. |
