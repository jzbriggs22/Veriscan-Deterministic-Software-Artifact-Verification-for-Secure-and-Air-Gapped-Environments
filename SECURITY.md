...
- The dependency tree is reviewed with `cargo audit` against the RustSec advisory database. The bundled GitHub Actions workflow (`.github/workflows/ci.yml`) runs `cargo fmt --check`, `cargo clippy -- -D warnings`, the full test suite, and a blocking `cargo audit` pass on every push and pull request. Advisories without an upstream fix may be ignored only via `.cargo/audit.toml`, each with a written justification.
